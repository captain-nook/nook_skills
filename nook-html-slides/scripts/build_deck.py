# -*- coding: utf-8 -*-
"""Template-driven editable PPTX builder for nook-html-slides.
...
STRICT_TEMPLATE_COMPONENTS = {
    ...
    "zine.hierarchy": {"four-layer", "timeline-like", "skill-structure", "five-layer-preview", ""},
...
TEMPLATE_CAPACITY = {
    "zine.process-cards": {"min": 3, "max": 5},
    "zine.timeline": {"min": 3, "max": 5},
    "zine.cycle": {"min": 3, "max": 4},
    "zine.hierarchy": {"min": 3, "max": 5},
    "zine.image-collage": {"min_images": 2, "max_images": 3},
}
...
def normalize_template_variant(page: Dict[str, Any]) -> None:
    """Normalize legacy page-form names to approved safe template variants."""
    if page.get("component") == "zine.hierarchy" and page.get("variant") == "network":
        page["variant"] = "timeline-like"
        if len(page.get("cards", []) or []) > 5:
            page["cards"] = (page.get("cards", []) or [])[:5]
        page["template_note"] = "network downgraded to safe layer stack; arbitrary connectors disabled"
    if page.get("component") == "zine.process-cards" and page.get("variant") in ["persona-cards", "action-path"]:
        if len(page.get("cards", []) or []) > 5:
            page["cards"] = (page.get("cards", []) or [])[:5]
        page["variant"] = "four-step" if len(page.get("cards", []) or []) >= 4 else "three-deliverables"
        page["template_note"] = "legacy process variant normalized to approved card template"
    if page.get("component") == "zine.hierarchy" and len(page.get("cards", []) or []) > 5:
        page["cards"] = (page.get("cards", []) or [])[:5]
        page["template_note"] = "hierarchy over capacity trimmed to 5 visible cards; create a larger template variant if needed"

def enforce_template_contract(page: Dict[str, Any]) -> None:
    """Fail fast when the confirmed page does not fit the approved template package.

    This is the safety rail learned from the v0.4-v0.7 failures: no freeform rescue
    layout, no arbitrary network, no auto-spread card grids.
    """
    no = page.get("page_no")
    component = page.get("component", "")
    variant = page.get("variant", "")
    if component not in STRICT_TEMPLATE_COMPONENTS:
        raise ValueError(f"page {no}: unsupported component {component}; add a template variant first")
    if variant not in STRICT_TEMPLATE_COMPONENTS[component]:
        raise ValueError(f"page {no}: variant {variant!r} is not approved for {component}; add it to the template package first")
    if variant == "network":
        raise ValueError(f"page {no}: arbitrary network layout is disabled; create an approved network template before use")
    cards = page.get("cards", []) or []
    cap = TEMPLATE_CAPACITY.get(component)
    if cap and "min" in cap:
        if not (cap["min"] <= len(cards) <= cap["max"]):
            raise ValueError(
                f"page {no}: {component} accepts {cap['min']}-{cap['max']} cards in the current template package; "
                f"got {len(cards)}. Split the page or create a new template variant."
            )
    if component == "zine.image-collage":
        slots = page.get("image_slots", []) or []
        if not (2 <= len(slots) <= 3):
            raise ValueError(f"page {no}: image-collage requires 2-3 image slots; got {len(slots)}")
def validate_plan(plan: Dict[str, Any]) -> None:
    progress("CHECK", "validating locked plan structure")
    pages = plan.get("pages", [])
    if not pages:
        raise ValueError("deck_plan has no pages")
    seen = set()
    for p in pages:
        normalize_template_variant(p)
        enforce_template_contract(p)
        no = p.get("page_no")
        if no in seen:
            raise ValueError(f"duplicate page_no: {no}")
        seen.add(no)
        if not p.get("component"):
            raise ValueError(f"page {no} missing component")
