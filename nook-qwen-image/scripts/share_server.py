#!/usr/bin/env python
"""Small HTTP wrapper for sharing nook-qwen-image with trusted users."""
...
spec = importlib.util.spec_from_file_location("nook_qwen_generate", GENERATE)
...
<head>
  ...
  <title>nook-qwen-image</title>
  <style>
    body { margin: 0; font-family: system-ui, -apple-system, BlinkMacSystemFont, "Microsoft YaHei", sans-serif; background: #f6f2eb; color: #161616; }
    main { max-width: 860px; margin: 0 auto; padding: 40px 20px 64px; }
    h1 { font-size: 34px; margin: 0 0 18px; }
    form { display: grid; gap: 14px; background: white; border: 1px solid #e7ded2; border-radius: 14px; padding: 20px; }
    label { display: grid; gap: 8px; font-weight: 700; }
    textarea, input, select, button { font: inherit; border-radius: 10px; border: 1px solid #d8cec1; padding: 12px 14px; }
...
<body>
  <main>
    <h1>nook-qwen-image</h1>
    <form id="form">
      <label>Prompt
        <textarea name="prompt" required>中文小红书封面背景，高级杂志风，干净构图，留标题区域，无文字，无 logo，无水印</textarea>
      </label>
      <label>Size
        <select name="size">
...
def authorized(handler: BaseHTTPRequestHandler, token_value: str) -> bool:
    ...
    header_token = handler.headers.get("X-Qwen-Image-Token", "")
...
def task_worker(task_id: str, prompt: str, size: str) -> None:
    ...
    try:
        ...
        image_path = generate_module.generate_image(prompt, size, str(output), timeout=600)
        with tasks_lock:
...
def make_handler(token_value: str):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            size = str(payload.get("size", "1080x1440")).strip()
            if not prompt:
                json_response(self, {"error": "prompt is required"}, HTTPStatus.BAD_REQUEST)
                return
            task_id = f"share_{int(time.time())}_{os.urandom(3).hex()}"
            with tasks_lock:
                tasks[task_id] = {
                    "task_id": task_id,
                    "status": "queued",
            ...
            json_response(self, {"task_id": task_id, "status": "queued", "estimated_wait": 60})
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Share nook-qwen-image through a small HTTP server.")
    ...
    parser.add_argument("--token", default=os.getenv("QWEN_IMAGE_SHARE_TOKEN", ""), help="Optional shared access token")
    args = parser.parse_args()

    server = ThreadingHTTPServer((args.host, args.port), make_handler(args.token))
    print(f"nook-qwen-image share server: http://{args.host}:{args.port}")
    if args.token:
