...
const outputDir = resolveOutputDir(process.env.IMAGE_OUTPUT_DIR);
...
async function saveImageBuffer(taskId, buffer, index, contentType = "image/png") {
  await fs.mkdir(outputDir, { recursive: true });
  const suffix = index !== undefined ? `_${index}` : "";
...
async function runImageTask(task) {
  ...
  try {
    if (task.mode === "image_to_image") {
      ...
      await fs.access(task.input_image_path);
    }

    if (config.dryRun) {
      await new Promise((resolve) => setTimeout(resolve, 1000));
      task.status = "succeeded";
      const dryPath = await saveBase64Png(task.task_id, DRY_RUN_PNG);
      task.image_path = dryPath;
      task.image_paths = [dryPath];
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        let savedPath;
        if (b64Json) {
          savedPath = await saveBase64Png(task.task_id, b64Json, items.length > 1 ? i : undefined);
        } else if (url) {
          const imageResponse = await fetch(url);
          if (!imageResponse.ok) throw new Error(`download failed HTTP ${imageResponse.status}`);
          const contentType = imageResponse.headers.get("content-type") || "image/png";
          const buffer = Buffer.from(await imageResponse.arrayBuffer());
          savedPath = await saveImageBuffer(task.task_id, buffer, items.length > 1 ? i : undefined, contentType);
        } else {
  ...
  async ({ task_id }) => {
    ...
    if (!task) {
      return jsonText({
        task_id,
        status: "not_found",
        error: "No task exists for this task_id in memory or persisted task storage"
      });
    }
