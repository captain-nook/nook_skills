...
# 方式 B：一键部署（自动写入 Agent 配置）
node setup.js sk-your-key-here https://your-relay-server
```
...
| `IMAGE_DRY_RUN` | 否 | `true` | 设置为 `false` 启用真实调用 |
...
生成图片默认保存在 `server/output/`。可通过 `IMAGE_OUTPUT_DIR` 修改。

该目录下的 `.png`/`.jpg` 文件已被 `.gitignore` 排除，不会提交。
