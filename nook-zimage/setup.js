...
console.log(FLAGS.remove ? "nook-zimage uninstall" : "nook-zimage setup");
console.log(FLAGS.remove
  ? "Removing MCP entries from detected agent configs and deleting .env."
  : "Writes your ModelScope key to local .env and configures the Z-Image MCP server when possible.");
...
if (FLAGS.remove) {
  ...
  // Share key with nook-qwen-image sibling if present.
  const siblingEnvPath = path.resolve(__dirname, "..", "nook-qwen-image", ".env");
...
if (FLAGS.remove) {
  ...
  console.log(configured > 0
    ? `Done. Configured ${configured} client(s). Restart your agent, then try "生一个美女 1024x1024".`
