#!/usr/bin/env python
"""nook-zimage CLI: ModelScope Z-Image Turbo text-to-image."""
...
def generate_image(prompt: str, size: str, output_file: Optional[str] = None, timeout: int = 300) -> str:
    load_env(ROOT / ".env")

    width, height = parse_size(size)
    output_dir = Path(os.getenv("ZIMAGE_OUTPUT_DIR", str(ROOT / "output")))
    ...
    if output_file:
        ...
        output = output_dir / f"zimage_{stamp}.jpg"
    output.parent.mkdir(parents=True, exist_ok=True)

    if os.getenv("ZIMAGE_DRY_RUN", "").lower() == "true":
    ...
    model = os.getenv("MS_IMAGE_MODEL", "Tongyi-MAI/Z-Image-Turbo")
    headers = {
    ...
    print(f"nook-zimage: submitting {width}x{height} image task")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate an image with ModelScope Z-Image Turbo.")
    parser.add_argument("prompt", help="Text prompt")
    parser.add_argument("--size", default="1024x1024", help="Image size, e.g. 1024x1024 or 720*1280")
