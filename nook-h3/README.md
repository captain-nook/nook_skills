...
容量不足、临时网络错误或推理失败也会逐次记录，并使用新种子按指数退避等待后重试。默认从 30 秒开始，最长等待 300 秒，可通过 `retry_backoff_seconds` 与 `retry_backoff_max_seconds` 调整；达到 `max_retries` 后进入 `rework_pending`，不会静默当作完成。
